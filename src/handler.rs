...
use crate::{
    settings::{self, State},
    AdminResponse, Message, Source, UserResponse,
};

pub type AsyncState = Arc<RwLock<State>>;

pub async fn handle_user_message(state: AsyncState, message: Message) -> Result<UserResponse> {
...
pub async fn handle_admin_message(state: AsyncState, content: String) -> Result<AdminResponse> {
